  ...
  if (req.isAuthenticated()) {
    try {
      ...
      let students = await Student.find({})
        ...
        // .populate([{ path: "batch", strictPopulate: false }])
        .populate({
          path: "interviews",
          populate: query1,
        })
      ...
      Employee.uploadedFile(req, res, async (err) => {
        ...
        if (req.file) {
          ...
          if (employee.avatarPath) {
            ...
            if (
              fs.existsSync(path.join(__dirname, "..", employee.avatarPath))
            ) {
              //Delete that Old Avatar
              fs.unlinkSync(path.join(__dirname, "..", employee.avatarPath));
            }
          }
          //Save the New Avatar
          //Saving the path of the uploaded file into the avatarPath field of the employee
          employee.avatarPath = Employee.filePath + "/" + req.file.filename;
        }
        //Save the Employee
        await employee.save();
